...
public class PaymentsService {
    ...
    public String makeRoyaltyPayment(int songId, int month, int year) {
        ...
        try {
            ...
            ResultSet royaltiesResultSet = statement.executeQuery("SELECT playCount * royaltyRate AS amount FROM songs WHERE songId = "+songId);
    ...
    public String undoMakeRoyaltyPayment(int songId) {
        Connection connection = genericDAO.createConnection();
        try {

            connection.setAutoCommit(false);
            Statement statement = connection.createStatement();
            ResultSet playCount = statement.executeQuery("SELECT playCount FROM songHistory WHERE songId="+songId+" AND month=4 AND year=2023");
            statement.executeQuery("DELETE FROM songHistory WHERE songId="+songId+" AND month=4 AND year=2023");
            playCount.next();
            statement.executeQuery("UPDATE songs SET playCount = "+playCount.getInt(1)+" WHERE songId ="+songId);
            ResultSet transaction = statement.executeQuery("SELECT transactionId FROM serviceAccount WHERE date=curdate() AND amount = 50 and type ='Debit'");
            statement.executeQuery("DELETE FROM serviceAccount WHERE date=curdate() AND amount = 50 and type ='Debit'");
            transaction.next();
            statement.executeQuery("DELETE FROM receives WHERE transactionId = "+transaction.getInt(1));
            statement.executeQuery("DELETE FROM distributesRoyalties where songId = "+songId+" AND date = curdate()");
